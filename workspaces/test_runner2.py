...
with patch('sentinel_omega.infrastructure.api.noaa.fetch_mag_field', return_value=_mock_mag_df()):
    with patch('sentinel_omega.infrastructure.api.noaa.fetch_solar_wind', return_value=_mock_wind_df()):
        with patch('sentinel_omega.infrastructure.api.noaa.fetch_kp_index', return_value=_mock_kp_df()):
            with patch('sentinel_omega.infrastructure.api.usgs.fetch_earthquakes', return_value=_mock_eq_df()):
                    ...
                    with patch('sentinel_omega.infrastructure.pipeline.data_pipeline.fetch_schumann_resonance', return_value=(7.95, 1.2)):
                        with patch('sentinel_omega.infrastructure.pipeline.data_pipeline.fetch_lod_series', return_value=pd.DataFrame({
                            ...
                            with patch('sentinel_omega.infrastructure.pipeline.data_pipeline.compute_lunar_phase_series', return_value=np.linspace(0, 1, 30)):
                                with patch('sentinel_omega.infrastructure.api.noaa.fetch_goes_xray', return_value=pd.DataFrame({
                                    'time_tag': pd.date_range('2024-01-01', periods=100, freq='1min'),
                                    'flux': np.random.uniform(1e-7, 1e-4, 100),
                                })):
                                    with patch('sentinel_omega.infrastructure.api.nasa_neo.fetch_neo_hazard_summary', return_value={'hazardous_count': 5, 'closest_hazardous_ld': 12.3}):
                                        with patch('sentinel_omega.infrastructure.api.noaa.fetch_electron_flux', return_value=pd.DataFrame({'time_tag': pd.date_range('2024-01-01', periods=71, freq='5min'), 'flux': np.random.uniform(100, 10000, 71)})):
