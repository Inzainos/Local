...
| 3 | `local/sentinel-omega` | Respaldos manuales versionados | **Parcial en `b639aa8`:** 40 archivos fuera (`*.bak*`, `launcher_hex_backup_1788196565/`) y el tarball `_SCHEMA_PARTS_BACKUP_PRE_FIX.tar.gz` archivado. **Sobreviven 2**, porque su nombre no casa con el glob `*.bak*`: `launcher.py.broken-2026-09-10` (31 KB) y `launcher_fixed.py` (39 KB) |
| 4 | `local/padron`, `local/sentinel-omega` | Symlinks de skills apuntando dentro de `venv/`/`.venv/` (gitignorado): rotos en cualquier clon | **Parcial en `b639aa8`:** el de `sentinel-omega` fue eliminado. **Siguen los dos de `local/padron`** (`.agents/skills/` y `.claude/skills/`), que no tiene agente asignado. El symlink `sentinel_omega/sentinel_omega/data → ../data` se conserva a propósito: resuelve bien |
